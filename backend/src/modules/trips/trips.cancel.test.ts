...
describe('cancelTrip unit', () => {
  test('rejects cancelling a DRAFT trip', async () => {
    ...
    await expect(svc.cancelTrip('t1', 'user-1')).rejects.toBeInstanceOf(InvalidTripTransitionError);
  });
  ...
  test('cancels a DISPATCHED trip and reverts resources', async () => {
    ...
    const res = await svc.cancelTrip('t2', 'user-1');
  ...
  test('rejects cancel when trip is COMPLETED', async () => {
    ...
    await expect(svc.cancelTrip('t3', 'user-1')).rejects.toBeInstanceOf(InvalidTripTransitionError);
  });
  ...
  test('rejects cancel when trip is already CANCELLED', async () => {
    ...
    await expect(svc.cancelTrip('t5', 'user-1')).rejects.toBeInstanceOf(InvalidTripTransitionError);
  });
  ...
  test('transaction rolls back when vehicle guarded update fails', async () => {
    ...
    await expect(svc.cancelTrip('t4', 'user-1')).rejects.toBeInstanceOf(ResourceUnavailableError);
  });
  ...
  test('transaction rolls back when driver guarded update fails', async () => {
    ...
    await expect(svc.cancelTrip('t6', 'user-1')).rejects.toBeInstanceOf(ResourceUnavailableError);
  });
});
